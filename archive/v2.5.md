...
- `auth` (object, required): Authentication requirements
  ...
  - `details` (object, optional): Auth-specific configuration
    - For `x402`: `{"network": "base-sepolia", "asset": "0x036CbD...", "currency": "USDC"}` (price determined at call time via x402 negotiation)
...
class DynamicDCAPAgent {
  ...
  async prepareAuth(authConfig) {
    switch (authConfig.type) {
      ...
      case 'x402':
        // Initialize payment stream (price determined at call time)
        const { network, asset } = authConfig.details;
        return await this.initializeX402Payment(network, asset);
